...
class MyProject:
    def __init__(self, root):
        ...
        self.lblCạnhđáy = tk.Label(self.root, text = 'Cạnh Đáy △')
        self.lblCạnhđáy.place (x=self.x, y = self.y )
        self.etyCạnhđáy = tk.Entry(self.root, width= self.width)
        self.etyCạnhđáy.place (x = self.x, y = self.y +20)

        self.lblChiềucao = tk.Label(self.root, text = 'Chiềucao △')
        self.lblChiềucao.place (x=self.x, y = self.y +40)
        self.etyChiềucao = tk.Entry(self.root, width= self.width)
        self.etyChiềucao.place (x = self.x, y = self.y +60)

        self.lblChieuDaiHCN = tk.Label(self.root, text = 'Chiều dài HCN')
        self.lblChieuDaiHCN.place (x = self.x, y= self.y+80)
        self.etyChieuDaiHCN = tk.Entry(self.root, width= self.width)
        self.etyChieuDaiHCN.place (x = self.x, y = self.y + 100)

        self.lblChieuRongHCN = tk.Label(self.root, text = 'Chiều rộng HCN')
        self.lblChieuRongHCN.place (x=self.x, y = self.y + 120)
        self.etyChieuRongHCN = tk.Entry(self.root, width= self.width)
        self.etyChieuRongHCN.place (x = self.x, y = self.y + 140)


        self.lblBankinh = tk.Label(self.root, text = 'Bán kính ◯')
        self.lblBankinh.place (x=self.x, y = self.y + 160)
        self.etyBankinh = tk.Entry(self.root, width= self.width)
        self.etyBankinh.place (x = self.x, y = self.y + 180)
        ...
        self.btnTinhDienTichHCN.place (x=self.x + 250, y=self.y + 180)
